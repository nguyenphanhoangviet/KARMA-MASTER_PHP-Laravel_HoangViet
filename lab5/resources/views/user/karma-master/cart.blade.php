                                    ...
                                    <tbody>
                                        @foreach ($cart as $item)
                                            <tr>
                                                ...
                                                <td>
                                                    <img src="{{ asset('imgs/products/' . $item['img']) }}"
                                                        alt="{{ $item['name'] }}" style="width: 50px; height: 50px;">
                                                </td>
                                                <td>
                                                    <p>{{ isset($item['size']) && !empty($item['size']) ? $item['size'] : 'N/A' }}
                                                    </p>
                                                </td>
                                                <td>
                            ...
                            <div class="shipping_box">
                                <form action="{{ route('calculateShipping') }}" method="POST">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="street" name="phone" class="form-control"
                            ...
                            <div class="checkout_btn_inner d-flex align-items-center">
                                ...
                                <form action="{{ route('pay.index') }}" method="POST">
                                    ...
                                    <input type="hidden" name="address"
                                        value="{{ session('address', old('address')) }}">
                                    <input type="hidden" name="province"
                                        value="{{ session('province', old('province')) }}">
                                    <input type="hidden" name="district"
                                        value="{{ session('district', old('district')) }}">
                                    ...
                                    <input type="hidden" name="total"
                                        value="{{ collect($cart)->sum(fn($item) => $item['price'] * $item['quantity']) + session('shipping_fee', 0) }}">
                                    <button type="submit" class="primary-btn"
                                        style="border: none !important; box-shadow: none !important; margin-top:10px; border-radius: 0px">Proceed
                                        to checkout</button>
                                </form>
                            </div>
                        </div>
            ...
            <div class="container">
                <div class="row">
                    <div class="col-12 text-center">
                        <h3 style="color: #9f8c8c">Bạn chưa có sản phẩm nào trong giỏ hàng</h3>
                    </div>
                </div>
            </div>
        </section>
